# -*- coding: utf-8 -*-
"""캡스톤디자인_RAG+string_Matching_v11.ipynb
...
VECTOR_DB_PATH = "/content/drive/MyDrive/CapStone/캡스톤/review_db_v13"
META_PATH = "/content/drive/MyDrive/CapStone/캡스톤/restaurant_metadata.json"

MENU_DATA_PATH = "/content/drive/MyDrive/CapStone/캡스톤/menu_data_final.csv"
...
def normalize_tokens(text, is_use_v=True):
    ...
    for w, p in okt_pos_tags:
        ...
        if p == "Noun":
            ...
            result.add(w)
        elif is_use_v and p == "Verb":  # 동사 포함 여부
            result.add(w)

    cleaned = set()
    for word in result:
        if word.endswith(("하다", "한")):
            base = re.sub(r"(하다|한)$", "", word)
            if len(base) >= 2:
                cleaned.add(base)
        cleaned.add(word)
...
def apply_time_filter(df, tokens):
    lunch_kw = ["점심", "런치", "브런치", "아점"]
    dinner_kw = ["저녁", "디너", "야식", "심야", "밤"]
    time_slot = None
...
STATION_CENTER_COORDINATES = {
    "성수": (37.5446, 127.0551),
    "서울숲": (37.5434, 127.0447),
    "뚝섬": (37.5472, 127.0474),
    "건대입구": (37.5403, 127.0694),
    "어린이대공원": (37.5480, 127.0747)
}
...
# print(temp_data)
# temp_prk = get_prk_data(temp_data["성수카페거리"])
...
def prepare_rag_env(
    ...
    print(f"메타데이터 로드 완료: {len(restaurant_metadata_df)}개")
    ...
    print(f"DB에서 메타데이터 추출: {len(vector_db_metadata_df)}개")
    ...
    print(f"병합 완료: {len(merged_restaurant_review_df)}개 매칭")
    ...
    print(f"최종 데이터셋 준비 완료: {len(merged_restaurant_review_df)}개")
...
def apply_category_filter(df, keywords):
    ...
    category_map = {
        ...
        "중식": ["중식", "짜장", "짬뽕", "탕수육", "훠궈", "마라샹궈", "중화요리", "마라탕", "양꼬치"],
        "양식": ["양식", "파스타", "스테이크", "브런치"],
        "술집": ["술집", "바", "이자카야", "펍", "포차", "소주", "맥주", "포장마차", "안주"],
        "간식": ["간식", "카페", "까페", "베이커리", "빵집", "베이글", "브런치", "디저트"],
        "분식": ["분식", "떡볶이", "즉떡", "김밥", "라면"],
    }

    simple_categories = [
        "구내식당", "치킨", "패스트푸드", "퓨전요리", "도시락",
        "아시아음식", "샤브샤브", "뷔페", "샐러드", "패밀리레스토랑"
    ]
...
def apply_attribute_filter(df, keywords):
    attr_map = {
        ...
        "ambience_score": ["분위기", "조용", "감성", "데이트", "아늑", "로맨틱", "기념일"],
        "price_score": ["저렴", "가성비", "합리적", "혜자"],
...
def retrieve_similar_reviews(query,
    ...
    # print(f"벡터DB 임베딩 로드 개수: {len(vector_subset_dict['embeddings'])}")
    ...
    # print(f"유사도 계산 결과 : {similarity_result_df.head(5)}")
...
def time_weight_with_retrieve_similar_reviews(query,
    ...
    # print(f"최신 리뷰 가중치 부여 결과 : {time_weighted_similarity_df.head(5)}")
...
def rag_recommend_reviews(query):
    ...
    print(f"raw similarity top-k 결과 : {retrieve_topk_df.head(5)}")
...
def rag_recommend_reviews_with_time_weight(query):
    ...
    print(f"time-weight top-k 결과 : {time_weight_topk_df.head(5)}")
...
def select_reviews_mmr(
    ...
    #langchain의 MMR 수식을 직접 반영(쿼리와의 유사도는 높게, 이미 선택된 리뷰들과 유사도는 낮게 조정)
...
# menu_store_token = pd.read_csv('/content/drive/MyDrive/Ehwa/캡스톤/menu_with_token.csv')
...
# print("station NaN restaurant_id 개수:", len(missing_ids))
# print("일부 확인:", missing_ids[:10])

# nan_ratio = menu_store_token['station'].isna().mean()
# print(f"station NaN 비율: {nan_ratio:.2%}")

# menu_store_token[menu_store_token['station'].isna()].head()

# menu_store_token.to_csv('/content/drive/MyDrive/Ehwa/캡스톤/menu_data_final.csv', index=False, encoding='utf-8-sig')
...
TIME_ATTR_WEIGHTS = {
  "lunch": {
        ...
        "spicy": 1.0,     # 너무 매운건 점심에 부담일 수 있음 (중립)
    },

    "dinner": {
        # 저녁 주력: 술, 회식, 거한 요리
        "alcohol_pair": 3.0,      # 술 안주 (강력)
        "beverage_alcohol": 3.0,  # 주류 (강력)
        "meat": 2.5,              # 고기
        "seafood": 2.5,           # 해산물
        "spicy": 2.5,             # 매운맛
        "hot": 2.0,               # 뜨거운 요리
        ...
        # 저녁 비선호
        "bakery": 0.6,            # 빵은 식사로 부족
        "dessert": 0.6,
        "diet": 0.8,              # 저녁 회식때 다이어트는 잠시...
    },
}
...
def preprocess_dataframe(df, query):
    if "뷔페" not in query:
      df = df[~(
          (df["category_1"].str.contains("뷔페", na=False)) |
          (df["menu_title"].str.contains("뷔페", na=False))
      )]
    if "세트" not in query:
      df = df[~(
          (df["menu_title"].str.contains("정식", na=False))|
          (df["menu_title"].str.contains("세트", na=False))
      )]
    return df
...
def score_menus(df, rating_weight=0.2):
    ...
    print("감지된 속성:", detected_attrs)
    print("감지된 시간대:", INTENT_STATE["time_slot"])
    ...
    # print(f"베이지안 30% {}")
    df["베이지안"] = rating_weight * df["rating_bayes"]

    df["속성가중치점수"] = 0.8 * df["temp_simple"]
    df["시간대가중치점수"] = 0.8 * df["match_score"]

    df["최종 속성가중치점수"] = 0.8 * df["temp_simple"] + rating_weight * df["rating_bayes"]
    df["score_attrs"] = 0.8 * df["match_score"] + rating_weight * df["rating_bayes"]


    df["rank_simple"] = df["최종 속성가중치점수"].rank(ascending=False, method='min').astype(int)
    ...
    def make_rank_change_str(row):
        ...
        if diff > 0:
            return f"{old_rank}위 ➡ {new_rank}위 (🔺{diff})"
        elif diff < 0:
            return f"{old_rank}위 ➡ {new_rank}위 (🔻{abs(diff)})"
        else:
            return f"{new_rank}위 (-)"

    df["rank_change"] = df.apply(make_rank_change_str, axis=1)

    print("[비교] 속성 가중치 로직 vs 속성+시간 가중치 로직 순위 변동 확인")
    cols = ["name","menu_title", "속성가중치점수", "시간대가중치점수", "최종 속성가중치점수", "score_attrs", "rank_change","베이지안"]

    print("\n--- 속성 가중치 점수(w=1) 기준 Top 3 ---")
    print(df.sort_values("최종 속성가중치점수", ascending=False)[cols].head(5))

    print("\n--- 속성 + 시간 가중치 점수 기준 Top 3 ---")
...
def score_menu_title(df, rating_weight=0.2):
    ...
    df["키워드매칭점수"] = 0.8 * df["score_title"]
    df["score_title"] = 0.8 * df["score_title"] + rating_weight * df["rating_bayes"]

    temp_cols = ["name", "menu_title","match_score","score_title","title_tokens","베이지안"]
...
def recommend_menu(query, w_attr=0.7, w_title=0.3):
    ...
    if INTENT_STATE["stations"]:
        ...
        print(f"menu data station filter 적용 전 : {len(base_df)} rows")
        display(base_df.head(5))
        base_df = base_df[base_df["station"].astype(str).isin(st)]
        print(f"menu data station filter 적용 후: {st} → {len(base_df)} rows")
        display(base_df.head(5))
...
SYSTEM_PROMPT = """
당신은 성수, 서울숲, 자양, 건대입구 지역의 맛집을 추천하는 챗봇입니다.
답변하기 전에 아래 조건들을 단계적으로 분석하고 최적의 결론을 도출하세요.
그러나 분석 과정은 사용자에게 공개하지 말고,
최종 결론만 명확하고 간결하게 제시하세요.

**반드시 제공된 JSON 데이터(context) 와 추가로 제공될 수 있는 (parking_info) 안의 정보만 사용하여 답변하세요.**
JSON에 없는 식당 / 메뉴 / 평가 / 특징을 생성하거나 추측하지 마세요.
정보가 부족할 경우 다음 문장을 그대로 출력합니다:
"관련 정보가 없습니다"
...
# ★ 내부 추론 프로세스 (최종 답변에 포함 금지)

답변을 생성하기 전에, 반드시 다음 단계를 내부적으로 수행하십시오:

1.  **질문 유형 분류 (Classification):** {question}과 chat_history를 검토하여 이 질문이 "처음 질문"인지 "후속 질문"인지 판별합니다.
  1.1. chat_history가 비어있으면 → “처음 질문”
  1.2. chat_history가 비어있지 않으면 → “후속 질문”

2.  **데이터 사용 계획 (Planning):**
    * "처음 질문": context의 JSON 데이터를 사용하여 **추천 로직**을 실행하고, 아래 지정된 **텍스트 출력 형식**에 따라 추천 목록을 생성합니다.
    * "후속 질문": context의 JSON 데이터 내에서 {question}에 직접적으로 관련된 정보만 검색하여 **자연스러운 대화** 형식으로 응답할 계획을 세웁니다.

3. "처음 질문" 규칙
  3.1. 추천 로직:
  - 리뷰 기반: similarity를 이용한 결과 기반
  - 메뉴 기반: menus, menus2 기반
  - 가격, 분위기, 거리 등 JSON에 없는 정보 언급 금지
  - "서로다른 식당" 모두 추천 (없으면 "없음" 명시)

  3.2.텍스트 출력 형식

  리뷰기반과 메뉴기반 추천 식당 1개 당 다음 항목을 **순서대로** 나열하여 추천 목록을 구성하며 ****3개 이상의 최대한 많은****식당을 추천합니다.(답변에 직접적으로 이 멘트를 하는건 금지)

  1) 추천 식당명: (식당 이름)
     - 추천 메뉴: (context에 있는 메뉴명들을 쉼표로 나열: 메뉴1, 메뉴2, 메뉴3...)
     - 메뉴 특징: (매운/국물/바삭함/가성비/분위기 등, menus/menus2 속성 필드가 있을 경우에만 요약)
     - Best review: "(review 항목이 있을 경우 similarity가 가장 높은 "text" 값을 원문 그대로“<...>” 형식으로 언급)
     - 가는 방법: (distance_m, distance_from_station 등 거리/시간 정보 활용하여 자연스럽게 서술)
     - 추천 이유: context에 있는 추천 식당의 모든 리뷰를 참고하여 추천 식당의 핵심 특징, 주 메뉴의 속성, 언급된 분위기를 기반으로 자연스럽게 작성
     - URL : kakao_url에 있는 링크를 클릭하면 연결할 수 있도록 함


  위 항목들 중, context에 해당 필드가 없으면 그 항목은 생략 합니다.


4. "후속 질문" 규칙

4.1. 답변 규칙
  후속 질문은 ** 기존에 제공된 {context}와 추가로 제공될 수 있는 parking_info와 chat_history 기반 대화**를 목표로 하며, 다음을 절대 금지합니다:
  - 새로운 식당, 메뉴, 리뷰 추가 생성
  - 추천 로직 / 필터링 / retrieval 재수행 (새로운 추천 목록 생성 금지)
  - ***처음 질문의 텍스트 출력 형식을 따르지 말것***

4.2. 텍스트 구성 가이드

  - 사용자가 "주차", "주차장", "차 가져가도 돼?" 등 주차 관련 질문을 하면:
    * FEWSHOT_PARKING 내용을 참고하여 친절하게 답변합니다
    * 여러 주차장이 있을 경우, 거리(distance)가 가깝거나 남은 자리(CPCTY - CUR_PRK_CNT)가 더 많은 곳을 중심으로 비교 설명
  - 날씨/인구 혼잡도 관련 후속 질문이면:
    * weather_info, pop_status_info 안에 있는 값만 사용해서 한두 문장으로만 현재 상황을 설명한 후, 후속질문에 맞는 답변으로 활용함
  - 이전 대화 목록인 chat_history를 종합하여 질문 내용을 추론

  - {context}에 없는 정보는 절대 추측/생성하지 말고,
    정말로 관련 필드가 없으면 "관련 정보가 없습니다"라고만 답변합니다.


5. 최종 검증 (Self-Verification)

5.1.  **출력 형식 강제:** "처음 질문"에 대한 답변은 **위에서 정의된 텍스트 추천 목록 형식**으로만 구성되어야 하며, **추천 식당명**부터 시작해야 합니다.
5.2. 부정적인 리뷰가 있는 경우에는 추천 식당에서 제외

----------------------------------------------------

# ★ 중요 요약 (Agent Goal)

- 처음 질문 → **추천 식당 목록** + 리뷰/메뉴/역/거리 등을 최대한 활용해서 최대한 많은 식당을 제안 (단, 모두 context 안에 있는 값만 사용)
- 후속 질문 → chat_history와 context를 기반으로 맥락 있게 보완 설명, **새로운 후보 생성 금지**, 주차/날씨/혼잡도는 parking_info, weather_info, pop_status_info 필드만 활용
"""

FEWSHOT_PARKING = """
[Few-shot Example #1 — "Parking info" 제공되는 경우 경우]
...
{{
  ...
  "retrieve_review": {{
    "restaurants": [{{ "restaurant_name": "성수우동" }}]
  }}
}}
...
Assistant:
네! 성수우동과 가장 가까운 주차장은 **성수1공영주차장**입니다.
- 총 98대 중 33대 주차 → 65대 남음
- 거리: 약 121m
- 요금: 1시간 1,000원, 30분 500원 추가
실시간 정보 제공됩니다.


[Few-shot Example #2 — "parking info" 없는 경우]
...
Assistant:
현재 제공된 XML 데이터에는 주차장 정보가 없습니다.
필요 시 주차 앱에서 실시간 확인을 추천드립니다.


[Few-shot Example #3 — 여러개 주차장 정보가 있는 경우]

System Context:
{{
  "parking_info": [
    {{ "PRK_NM": "성수1공영주차장", "CPCTY": "98", "CUR_PRK_CNT": "90", "distance": 200 }},
    {{ "PRK_NM": "성수2공영주차장", "CPCTY": "120", "CUR_PRK_CNT": "50", "distance": 350 }}
  ]
}}
...
Assistant:
가장 가까운 곳은 **성수1공영주차장(200m)** 이지만 남은 공간은 8대입니다.
여유 있는 주차를 원한다면 **성수2공영주차장(남은 자리 70대)** 을 추천드립니다.
"""
...
def is_weather_query(text):
    weather_keywords = [
        "날씨", "기온", "온도", "추워", "더워", "더운", "추운", "시원한",
        "따뜻한", "비와", "눈와", "맑아", "흐려", "강수", "바람", "습도",
        "미세먼지", "먼지", "바깥", "실내", "밖에", "야외", "루프탑", "테라스"
    ]
...
#     # evt.value는 선택된 텍스트입니다.
#     return gr.update(visible=True), f"선택된 위치: {evt.value}"

# def step2_selected(evt: gr.SelectData):
#     return gr.update(visible=True), f"선택된 목적: {evt.value}"

# def step3_selected(evt: gr.SelectData):
#     return gr.update(visible=True), f"선택된 메뉴: {evt.value}"
...
with gr.Blocks(theme="soft", css="body {zoom: 1.3;}") as demo:
    gr.Markdown("### 🥚 메추리알!! 성수/서울숲/건대 맛집 추천 챗봇")

    with gr.Row():
        mode_selector = gr.Radio(
            ["자율 검색형 (키워드)", "가이드라인형 (시나리오)"],
            label="사용 모드 선택",
            value="자율 검색형 (키워드)"
        )
    ...
    with gr.Row():
        msg = gr.Textbox(label="질문 입력", scale=4)
        submit = gr.Button("전송", scale=1)
        reset_btn = gr.Button("새 대화 시작", scale=1)

    with gr.Accordion("분석 데이터", open=True):
        with gr.Tabs():
            with gr.TabItem("실시간 데이터"):
                with gr.Row():
                    pop_status_df = gr.Dataframe(label="실시간 인구데이터")
                    weather_df = gr.Dataframe(label="실시간 날씨데이터")

            with gr.TabItem("RAG 추천"):
                with gr.Row():
                    rag_retrieve_df = gr.Dataframe(label="1차: 리뷰 기반 추천 (Retrieve)")
                    rag_time_weight_df = gr.Dataframe(label="2차: 시의성 가중치 (Time Weight)")
                    rag_mmr_df = gr.Dataframe(label="3차: 다양성 확보 (MMR)")

            with gr.TabItem("메뉴 추천"):
                with gr.Row():
                    menu_attr_out = gr.Dataframe(label="추출된 메뉴 속성", col_count=(3, "fixed"))
                    menu_title_out = gr.Dataframe(label="메뉴 타이틀", col_count=(3, "fixed"))
